...
export const ShopScreen = ({ route, navigation }) => {
  ...
  const { idCateg } = route.params;

  useEffect(() => {
    const baseUrl2 = `${baseUrl}products/search/findByIdCategorie?idCategorie=${idCateg}`;
    const url = `${baseUrl2}`;
    fetch(url, {
      headers: {},
    })
      ...
      <CategoriesTitleContainer>
        ...
        <SectionEnd>
          ...
          <TouchableOpacity style={{ marginTop: 5 }}>
            <Ionicons name="funnel-outline" size="25px" color="grey" />
          </TouchableOpacity>
        </SectionEnd>
      </CategoriesTitleContainer>
      <ScrollView>
          ...
          {prods.map((prod) => {
            return (
              <ProductCard>
                <TouchableOpacity
                  onPress={() =>
                    navigation.navigate("ProductDetails", { produs: prod })
                  }
                >
